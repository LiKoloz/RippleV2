<script>
    ...
    const res = await fetch(`http://80.242.57.142:8080/posts/all/noncheked/${page}`, {
      headers: {
   ...
   async function reduce_page() {
    ...
    async function accept_all_posts() {
        let res = await fetch("http://80.242.57.142:8080/posts/accept/all", {
            method: "GET"
        })
        if (res.ok){
            posts = []
    ...
    async function reject_all_posts() {
        let res = await fetch("http://80.242.57.142:8080/posts/reject/all", {
            method: "DELETE"
        })
        if (res.ok){
            posts = []
    ...
    async function accept_post(id) {
        let res = await fetch(`http://80.242.57.142:8080/posts/accept/${id}`, {
            method: "GET"
        })
    ...
    async function reject_post(id) {
        let res = await fetch(`http://80.242.57.142:8080/posts/reject/${id}`, {
            method: "DELETE"
        })
        if (res.ok){
            posts = posts.filter(p => p.id != id)
